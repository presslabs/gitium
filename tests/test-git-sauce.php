...
class Test_Git_Sauce extends WP_UnitTestCase
{
  ...
  function test_has_filter_upgrader_post_install() {
    $this->assertGreaterThan( 0,
      has_filter('upgrader_post_install', 'git_upgrader_post_install') );
  }
  
  function test_has_action_upgrader_process_complete() {
    $this->assertGreaterThan( 0,
      has_action('upgrader_process_complete', 'git_pull_and_push') );
  }
  ...
  function test_git_format_message() {
    ...
    $this->assertGreaterThan( '', _git_format_message('name') );
  }
  
  function test_has_action_activated_plugin_git_check_post_activate_modifications() {
    $this->assertGreaterThan( 0,
      has_action('activated_plugin','git_check_post_activate_modifications') );
  }
  
  function test_has_action_deactivated_plugin_git_check_post_deactivate_modifications() {
    $this->assertGreaterThan( 0,
      has_action('deactivated_plugin','git_check_post_deactivate_modifications') );
  }
  
  function test_has_action_git_check_for_plugin_deletions() {
    $this->assertGreaterThan( 0,
      has_action('load-plugins.php', 'git_check_for_plugin_deletions') );
  }
  
  function test_has_action_git_check_for_themes_deletions() {
    $this->assertGreaterThan( 0,
      has_action('load-themes.php','git_check_for_themes_deletions') );
  }

  function test_git_module_by_path() {
    set_transient('git_versions', array(
      'plugins' => array(
